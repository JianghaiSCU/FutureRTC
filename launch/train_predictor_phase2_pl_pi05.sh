...
# Usage: launch/train_predictor_phase2_pl_pi05.sh <gpu> <phase1_checkpoint> [output_dir]
set -euo pipefail
GPU=${1:?usage: train_predictor_phase2_pl_pi05.sh <gpu> <phase1_ckpt> [output_dir]}
BASE=${2:?phase-1 checkpoint required}
OUT=${3:-outputs/predictor_pi05_phase2_pl}
...
"$PY" predictor/train.py \
  --backbone pi05 --data-dir "$BANK" --state-sidecar "$SIDECAR" --sidecar "$SIDECAR" \

#!/usr/bin/env bash
# Build the per-frame visual-latent bank for smolvla.
# Usage: launch/collect_smolvla.sh <gpu> [worker_id] [num_workers]
# Run one process per worker_id, each on its own GPU, all writing to the same OUTPUT_DIR.
set -euo pipefail
GPU=${1:?usage: collect_smolvla.sh <gpu> [worker_id] [num_workers]}
WORKER_ID=${2:-0}
NUM_WORKERS=${3:-1}
...
OUTPUT_DIR=${OUTPUT_DIR:-outputs/latent_bank_smolvla}

...
# Usage: launch/train_predictor_phase1_pi05.sh <gpu> [output_dir]
set -euo pipefail
GPU=${1:?usage: train_predictor_phase1_pi05.sh <gpu> [output_dir]}
OUT=${2:-outputs/predictor_pi05_phase1}
REL=$(cd "$(dirname "$0")/.." && pwd)
PY=${PYTHON:-python3}
BANK=${BANK_DIR:-outputs/latent_bank_pi05}
...
"$PY" predictor/train.py \
  --backbone pi05 --data-dir "$BANK" --state-sidecar "$SIDECAR" \
  --corrector-checkpoint "$CORRECTOR" --output-dir "$OUT" \
  --steps 300000 --batch-size 64 --lr 1e-4 --lr-schedule cosine --lr-min 1e-6 \
  --mse-weight 1.0 --feature-weight "$FEATURE_WEIGHT" --policy-weight 0 \
  --max-delay 20 --num-workers 16 --seed 0 --ckpt-every 10000 --device cuda

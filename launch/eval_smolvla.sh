#!/usr/bin/env bash
# Multi-delay LIBERO eval of predictor + corrector for smolvla.
# All delays run concurrently in one env batch; suites run sequentially on one GPU.
# Usage: launch/eval_smolvla.sh <gpu> <predictor_ckpt> <out_base> [suite:max_steps ...]
# Default suites: libero_spatial:220 libero_object:280 libero_goal:300 libero_10:520
set -euo pipefail
GPU=${1:?usage: eval_smolvla.sh <gpu> <predictor_ckpt> <out_base> [suite:max_steps ...]}
...
for JOB in "${SUITES[@]}"; do
  ...
  echo "[eval_smolvla] $SUITE (max_steps=$MAXSTEPS) -> $OUT"
  ...
  "$PY" eval/driver.py \
    --backbone smolvla --policy-path "$POLICY" \
...
echo "[eval_smolvla] all suites done -> $OUTBASE"

...
|------|----------------|
| OS | Ubuntu 22.04 |
| GPU | ≥ 24 GB VRAM for LIBERO evaluation; predictor training scales with batch size |
| Python | 3.10+ |
...
**SmolVLA-450M** checkpoint. The backbone is never modified by this code.
...
|---|---|---|---|
| 1 — reconstruction | `mse 1.0 + feature 1.0` | lr 1e-4, batch 64, 300k steps | lr 3e-4, batch 256, 150k steps |
| 2 — policy consistency | `+ policy 10.0`, resumed from phase 1 | lr 1e-4, batch 32, 100k steps | lr 1e-4, batch 32, 100k steps |
...
Identical across backbones except the latent width; all values are module constants in
`predictor/model.py`, not flags.

| | |
|---|---|
| hidden dim | 256 |
| action encoder | 256 wide, 4 heads, 2 layers |
| streams (cameras) | 2, 64 tokens each (8×8 grid) |
| max delay | 20 |
| trunk block spec | `rrcccc` (per-token residual FFN / depthwise conv) |
| transport | continuous warp, max offset 1.0, max gain delta 1.0 |
| innovation decoder | `rrc` at width 384, conditioned on the initial frame |
| state dim | 8 (from the state correction module) |

| Backbone | Latent dim | Parameters |
|---|---|---|
| π₀.₅ | 2048 | 6,407,061 |
| SmolVLA-450M | 960 | 5,146,069 |

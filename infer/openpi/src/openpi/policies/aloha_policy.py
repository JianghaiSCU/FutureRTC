...
SONGLING_GRIPPER_MIN = 0.0
SONGLING_GRIPPER_MAX = 0.1

def _gripper_to_angular(value):
    # 先把异常值或超调值强行按死在 0 到 0.1 之间
    value = np.clip(value, SONGLING_GRIPPER_MIN, SONGLING_GRIPPER_MAX)
    # 再进行映射
    return _normalize(value, min_val=SONGLING_GRIPPER_MIN, max_val=SONGLING_GRIPPER_MAX)

def _gripper_from_angular(value):
    # 模型输出后, 先还原成物理值
    value = _unnormalize(value, min_val=SONGLING_GRIPPER_MIN, max_val=SONGLING_GRIPPER_MAX)
    # 给输出也加一道保险, 防止模型抽风输出超限指令烧坏电机
    return np.clip(value, SONGLING_GRIPPER_MIN, SONGLING_GRIPPER_MAX)

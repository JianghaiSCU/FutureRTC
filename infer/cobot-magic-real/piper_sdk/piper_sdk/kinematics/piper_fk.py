...
class C_PiperForwardKinematics:
    ...
    def CalFK(self, cur_j):
        ...
        j_pos.append(self.tmat_to_eula(_Rt[0]))  # Euler angles for link1
        j_pos.append(self.tmat_to_eula(R02))  # Euler angles for link2
        j_pos.append(self.tmat_to_eula(R03))  # Euler angles for link3
        j_pos.append(self.tmat_to_eula(R04))  # Euler angles for link4
        j_pos.append(self.tmat_to_eula(R05))  # Euler angles for link5
        j_pos.append(self.tmat_to_eula(R06))  # Euler angles for link6

        return j_pos
    
    def tmat_to_eula(self,T: np.ndarray):
        pos, sixd= self.tmat_to_xyz6d(T)
        rot = self.sixd_to_euler(sixd,seq = "XYZ",degrees=True)
        j_pos = [pos[0],pos[1],pos[2],rot[0],rot[1],rot[2]]
        return j_pos



    def tmat_to_xyz6d(self,T: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
        """
        参数
        ----
        ...
        返回
        ----
        t : ndarray, shape=(3,), dtype=float32
            平移向量 (x, y, z)
        sixd : ndarray, shape=(6,), dtype=float32
            6D 旋转表示，顺序 [r11,r21,r31,r12,r22,r32]（即 R[:,0] 和 R[:,1]）
        """
        ...
        # 3. 旋转矩阵 → 6D（前两列拉平）
        sixd = R_mat[:, :2].T.ravel()

        return t.astype(np.float32), sixd.astype(np.float32)
    
    def sixd_to_euler(self,sixd: np.ndarray,
                    seq: str = 'XYZ',
